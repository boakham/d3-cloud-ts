...
export namespace D3CloudTs {
  type canvasType<T> = T extends HTMLCanvasElement | (() => HTMLCanvasElement)
    ? CloudData
    : () => HTMLCanvasElement;
  ...
  type textFunctorType<T> = T extends
    | String
    | ((_: any, d: Word, i: number) => String)
    ? CloudData
    : (_: any, d: Word, i: number) => String;

  type numberFunctorType<T> = T extends
    | number
    | ((_: any, d: Word, i: number) => number)
    ? CloudData
    : (_: any, d: Word, i: number) => number;

  type sizeType<T> = T extends number[] ? CloudData : [number, number];

  type spiralType<T> = T extends
    | String
  ...
  export interface CloudData {
    ...
    font?: <T extends String | ((_: any, d: Word, i: number) => String)>(
      _?: T
    ) => textFunctorType<T>;
    fontStyle?: <T extends String | ((_: any, d: Word, i: number) => String)>(
      _?: T
    ) => textFunctorType<T>;
    fontWeight?: <T extends String | ((_: any, d: Word, i: number) => String)>(
      _?: T
    ) => textFunctorType<T>;
    rotate?: <T extends number | ((_: any, d: Word, i: number) => number)>(
      _?: T
    ) => numberFunctorType<T>;
    text?: <T extends String | ((_: any, d: Word, i: number) => String)>(
      _?: T
    ) => textFunctorType<T>;
    spiral?: <T extends String | ((size: number[]) => (t: number) => number[])>(
      _?: T
    ) => spiralType<T>;
    fontSize?: <T extends number | ((_: any, d: Word, i: number) => number)>(
      _?: T
    ) => numberFunctorType<T>;
    padding?: <T extends number | ((_: any, d: Word, i: number) => number)>(
      _?: T
    ) => numberFunctorType<T>;
  ...
  export class Cloud {
    ...
    protected board: number[] = this.zeroArray(
      (this.size[0] >> 5) * this.size[1]
    );
    protected bounds: { x: number; y: number }[] | null = null;
    protected contextAndRatio!: {
      context: CanvasRenderingContext2D;
      ratio: number;
    };
    protected dataSizeChanges: boolean = false;
    protected biggestSize: number = 0;
    ...
    constructor() {
      this.config.start = () => {
        ...
        let step = () => {
          let start = Date.now();
          while (
            Date.now() - start < this.timeInterval &&
            ++i < n &&
            this.timer
          ) {
            let d: Word = data[i];
      ...
      this.config.canvas = <T>(_?: T) => {
        return <canvasType<T>>(
          (_ ? ((this.canvas = this.functor(_)), this.config) : this.canvas)
        );
      };
      ...
      this.config.timeInterval = <T extends number | null>(
        _?: T
      ): timeInvervalType<T> => {
        return <timeInvervalType<T>>(
          (_
      ...
      this.config.size = <T extends number[]>(_?: T) => {
        return <sizeType<T>>(
          (_ ? ((this.size = [+_[0], +_[1]]), this.config) : this.size)
        );
      };

      this.config.font = <
        T extends String | ((_: any, d: Word, i: number) => String)
      >(
        _?: T
      ) => {
        return <textFunctorType<T>>(
          (_ ? ((this.font = this.functor(_)), this.config) : this.font)
        );
      };
      ...
      this.config.rotate = <
        T extends number | ((_: any, d: Word, i: number) => number)
      >(
        _?: T
      ) => {
        return <numberFunctorType<T>>(
          (_ ? ((this.rotate = this.functor(_)), this.config) : this.rotate)
        );
      };
      ...
      >(
        ...
        return <textFunctorType<T>>(
          (_ ? ((this.text = this.functor(_)), this.config) : this.text)
        );
      };

      this.config.spiral = <
        T extends String | ((size: number[]) => (t: number) => number[])
      >(
        _?: T
      ) => {
        return <spiralType<T>>(
          (_
            ? ((this.spiral =
                <String>_ == 'archimedean' || <String>_ == 'rectangular'
                  ? this.spirals[<'archimedean' | 'rectangular'>_]
                  : <(size: number[]) => (t: number) => number[]>_),
              this.config)
            : this.spiral)
        );
      };
      ...
      >(
        ...
        return <numberFunctorType<T>>(
          (_ ? ((this.fontSize = this.functor(_)), this.config) : this.fontSize)
        );
      };
      ...
      >(
        ...
        return <numberFunctorType<T>>(
          (_ ? ((this.padding = this.functor(_)), this.config) : this.padding)
        );
      };

      this.config.random = <T extends () => number>(_?: T) => {
        return <randomType<T>>(
          (_ ? ((this.random = _), this.config) : this.random)
        );
      };
    ...
    protected cloudSprite(
      contextAndRatio: { context: CanvasRenderingContext2D; ratio: number },
      d: Word,
      data: Word[],
      di: number
    ) {
